...
var Select=React.createClass({
    ...
    componentWillReceiveProps:function(nextProps){
        ...
        if (nextProps.defaultValue !== this.props.defaultValue) {
            this.setState({
                defValue:nextProps.defaultValue
            })
        }
    },
    ...
    render:function(){
        ...
        if(data&&data.length>0){
            ...
            data.forEach(function(d){
                if($.inArray(d.value, defaultValue)){
        ...
        return (<span>{this.props.disName}<select name={name} style={this.props.style} value={defaultValue} multiple={this.props.multiple} className={className} onChange={this._onChange}>{options}</select></span>);
    }
});
