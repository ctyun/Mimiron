 ...
 * @param {function} clickTab 点击标签的回调函数, 绑定一个参数为id
 * @example
...
var Tabs=React.createClass({
    ...
    render:function(){
        ...
        try{
            this.props.children.forEach(function(node){
                ...
                ul.push (
                    <li className={node.props.isActive?"active":null}>
                      <a href={href} data-toggle="tab" onClick={self.clickTab.bind(null,node.props.id)}>
                         {node.props.title}
                      </a>
                    </li>
                    );
            ...
            return <div>Tabs component needs more than one children!{e}</div>
        }
