...
var TencentMap = React.createClass({
	...
	getInitialState: function() {
		return {
			...
			mapWidth:"100%",
			zoom:7,
			mapCenter:{x:116.397128,y:39.916527},
		};
	},
	...
	initTencentMap : function() {
		...
		if(this.props.centerPoint){
			...
			//默认中心点
			centerPoint = new qq.maps.LatLng(this.state.mapCenter.y,this.state.mapCenter.x);
		}
		//设置放大倍数
	...
	setNewMarks : function(markers){
		...
		//设置地图中心点
		if(markers.length > 0){
			this.state.mapCenter.x = markers[0].x;
			this.state.mapCenter.y = markers[0].y;
			TencentMap.map[this.props.id].panTo(new qq.maps.LatLng(markers[0].y,markers[0].x));
		}
